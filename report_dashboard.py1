...
def fetch_and_build_master_ledger():
    ...
    # Ensure datetimes are standardized for filter engines
    df_orders["created_at_dt"] = pd.to_datetime(df_orders["created_at"], errors='coerce')
    ...
    # Process Shiprocket matches if data exists
    ...
    for _, order in df_orders.iterrows():
        ...
        # Pull sub-row items for this specific order container
        o_items = df_items[df_items["order_id"] == order_id] if not df_items.empty else pd.DataFrame()
        # Pull tracking adjustments based on our clean wildcard match number
        ...
        # Zip rows together cleanly into matching line counts rather than multiplying them
        for i in range(max_sub_rows):
            row_data = {
                # --- MASTER ORDER LEVEL DETAILS ---
                "Name": order.get("name"),
                "Created at": order.get("created_at"),
                "created_at_dt": order.get("created_at_dt"), # kept internally for date filter controls
                ...
                "Shipping": order.get("total_shipping_price_set"), # handles shipping rates
            ...
            # Map item parameters sequentially onto rows
            if i < len(o_items):
                item = o_items.iloc[i]
            ...
            # Map Shiprocket tracking sequentially onto rows
            if i < len(o_ships):
                ship = o_ships.iloc[i]
...
if raw_ledger.empty:
    ...
    if filter_type == "By Exact Calendar Dates":
        ...
        # Build strict dynamic selectors for Month and Year
    ...
    # Count unique order references instead of expanded sub-row length
    ...
    if not filtered_df.empty and "Total" in filtered_df.columns:
        # Avoid double-counting orders by calculating sum across unique order structures only
    ...
    # Secure columns format handling
