...
@st.cache_data
def generate_excel_with_merged_cells(df_json):
    """
    ...
    df = df.sort_values(by=["Created at", "shopify_lineitem_id", "shiprocket_shipment_id"], ascending=[False, True, True]).copy()
    ...
    for idx, row in df.iterrows():
        ...
        row_style = order_fill_map.get(order_name, "normal")
        ...
        for col_idx in range(1, len(EXPORT_COLUMNS) + 1):
            ...
            # Apply dynamic cell background fills
            if row_style == "yellow":
...
def render_html_merged_dashboard(raw_df):
    """Compiles standard data matrices directly into an HTML table with correct rowspan layout."""
    if raw_df.empty:
    ...
    html = """
    <style>
        .merged-table { width: 100%; border-collapse: collapse; font-family: 'Segoe UI', Geneva, Verdana, sans-serif; font-size: 13px; }
        .merged-table th { background-color: #1F4E78; color: white; padding: 10px; border: 1px solid #D9D9D9; position: sticky; top: 0; }
        .merged-table td { padding: 8px; border: 1px solid #E2E8F0; vertical-align: middle; text-align: center; color: #333333; }
    ...
    for _, row in df.iterrows():
        order_name = row.get("Name")
        row_class = order_fill_map.get(order_name, "row-normal")
...
if raw_df.empty:
    ...
    # --- CACHED DOWNLOAD TRIGGER ---
    st.sidebar.markdown("---")
    with st.spinner("📦 Pre-rendering Excel row-merges..."):
        # Convert dataframe to json text block to allow fast, stable hash caching inside memory matrix
        df_json = raw_df.to_json()
        excel_binary_data = generate_excel_with_merged_cells(df_json)
